...
class DouYinDownloader(object):
    ...
    def run(self):
        ...
        html_url = share_url_parse.scheme + "://" + share_url_parse.netloc + share_url_parse.path + "?" + share_url_parse.query;
            ...
            if response.status_code == 200:
                ...
                with open(video_name + ".mp4", "wb") as file:
                    for data in response.iter_content(chunk_size=chunk_size):
                        ...
                        sys.stdout.write(
                            '  [下载进度]:%.2f%% %s' % (float(size / content_size * 100), video_name + '.mp4 \r'))
                        sys.stdout.flush()
